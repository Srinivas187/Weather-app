...
function HomeTab({ weather, location, unit, setUnit, onRefresh }) {
  ...
  const hours = (hourly?.time || []).slice(startIdx, startIdx + 24).map((t, i) => ({
    ...
    icon:  getWMO(hourly.weather_code[startIdx + i]).emoji,
...
export default function App() {
  ...
  const loadWeather = useCallback(async (lat, lon, loc) => {
    ...
    try {
      ...
      // Always store lat/lon in location so refresh can re-use them
      setWeather(data);
    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords: { latitude, longitude } }) => {
        try {
          ...
          const name = g.address?.city || g.address?.town || g.address?.village || 'My Location';
          await loadWeather(latitude, longitude, { name, country: g.address?.country || '' });
        } catch { await loadWeather(latitude, longitude, { name:'My Location', country:'' }); }
      },
